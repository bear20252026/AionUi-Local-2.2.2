...
pub(super) async fn build(
    ...
    // Resolve the effective model id first. aionrs team agents carry no real
    // provider_id (the built-in "aionrs" id is only pushed by the login/account
    // flow, which is absent in local/offline mode), so we use the model name as
    // the stable lookup key for the fallback below.
    let model_id = model
        .use_model
        .as_deref()
        .filter(|s| !s.is_empty())
        ...
        .map_err(|e| AgentError::internal(format!("Failed to load provider config: {e}")))?
    {
        Some(row) => row,
        None => {
            // No provider row for this id (e.g. the built-in "aionrs" id does
            ...
            deps
                ...
                .ok_or_else(|| {
                    AgentError::bad_request(format!(
                        "Provider '{provider_id}' not found and no provider serves model '{model_id}'"
                    ))
                })?
        }
    };
