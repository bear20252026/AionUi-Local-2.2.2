...
pub trait IProviderRepository: Send + Sync {
    ...
    /// Finds the first enabled provider that serves `model_id`, or `None`.
    ///
    /// Used as a fallback when a model is referenced by its name alone instead
    /// of an explicit provider id (e.g. built-in "aionrs" agents in local /
    /// offline mode, where the login flow never pushed a dedicated provider
    /// row). Implementations match `model_id` against each provider's `models`
    /// list. The default returns `None`; `SqliteProviderRepository` overrides it.
    async fn find_by_model(&self, _user_id: &str, _model_id: &str) -> Result<Option<Provider>, DbError> {
        Ok(None)
    }
